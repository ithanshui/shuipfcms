...
class Shuipf extends TagLib
{
    ...
    protected $tags = array(
            ...
            // 上一篇
            'pre' => array(
                    'attr' => 'catid,id,target,msg,field',
                    'close' => 0
            ),
            // 下一篇
            'next' => array(
            ...
            // SQL标签
            'get' => array(
                    "attr" => 'sql,cache,page,dbsource,return,num,pagetp,pagefun,table,order,where',
                    'level' => 3
            ),
            // 推荐位标签
            'position' => array(
                    'attr' => 'action,cache,num,return,posid,catid,thumb,order,where',
                    'level' => 3
            ),
            // 评论标签
            'comment' => array(
                    'attr' => 'action,cache,num,return,catid,hot,date',
                    'level' => 3
            ),
            // Tags标签
            'tags' => array(
                    'attr' => 'action,cache,num,page,pagetp,pagefun,return,order,tag,tagid,where',
                    'level' => 3
            ),
            // sp模块调用标签
            'spf' => array(
                    'attr' => 'module,action,cache,num,page,pagetp,pagefun,return,where,order',
                    'level' => 3
            ),
            // 内容标签
            'content' => array(
                    'attr' => 'action,cache,num,page,pagetp,pagefun,return,where,moreinfo,thumb,order,day,catid,output',
                    'level' => 3
            )
    );
    ...
    public function _admintemplate ($attr, $content)
    {
        ...
        if ($counts < 2) {
            ...
            if ($counts < 3) {
                ...
                $file_path = "$file[0]/" . C('DEFAULT_V_LAYER') .
                         "/{$file[1]}/{$file[2]}";
            }
        // 模板路径
    ...
    {
        $cacheIterateId = to_guid_string(array(
                $tag,
                $content
        ));
        // 缓存时间
        ...
        $parsestr .= ' if ($_html) { S("' . $cacheIterateId . '", $_html, ' .
                 $cache . ');}';
        $parsestr .= ' echo $_html; ';
        $parsestr .= ' } ';
        $parsestr .= ' ?>';
    ...
    public function _pre ($tag, $content)
    {
        ...
        $parsestr = '<?php ';
        $parsestr .= ' $_pre_r = \Content\Model\ContentModel::getInstance(getCategory(' .
                 $tag['catid'] . ',"modelid"))->where(array("catid"=>' .
                 $tag['catid'] . ',"status"=>99,"id"=>array("LT",' . $tag['id'] .
                 ')))->order(array("id" => "DESC"))->field("id,title,url")->find(); ';
        if ($field) {
            $parsestr .= ' echo $_pre_r?$_pre_r["' . $field . '"]:""';
        } else {
            $parsestr .= ' echo $_pre_r?"<a class=\"pre_a\" href=\"".$_pre_r["url"]."\" ' .
                     $target . '>".$_pre_r["title"]."</a>":"' .
                     str_replace('"', '\"', $msg) . '";';
        }
        $parsestr .= ' ?>';
        return $parsestr;
    ...
    public function _next ($tag, $content)
    {
        ...
        $parsestr = '<?php ';
        $parsestr .= ' $_pre_n = \Content\Model\ContentModel::getInstance(getCategory(' .
                 $tag['catid'] . ',"modelid"))->where(array("catid"=>' .
                 $tag['catid'] . ',"status"=>99,"id"=>array("GT",' . $tag['id'] .
                 ')))->order(array("id" => "ASC"))->field("id,title,url")->find(); ';
        if ($field) {
            $parsestr .= ' echo $_pre_n?$_pre_n["' . $field . '"]:""';
        } else {
            $parsestr .= ' echo $_pre_n?"<a class=\"pre_a\" href=\"".$_pre_n["url"]."\" ' .
                     $target . '>".$_pre_n["title"]."</a>":"' .
                     str_replace('"', '\"', $msg) . '";';
        }
        $parsestr .= ' ?>';
        return $parsestr;
    ...
    public function _navigate ($tag, $content)
    {
        ...
        if (is_numeric($catid)) {
            ...
            // 获取当前栏目的 父栏目列表
            $arrparentid = array_filter(
                    explode(',', 
                            getCategory($catid, 'arrparentid') . ',' . $catid));
            foreach ($arrparentid as $cid) {
                $parsestr[] = '<a href="' . getCategory($cid, 'url') . '" ' .
                         $target . '>' . getCategory($cid, 'catname') . '</a>';
            }
            $parsestr = implode($space, $parsestr);
        } else {
            $parsestr = '';
            $parsestr .= '<?php';
            $parsestr .= '  $arrparentid = array_filter(explode(\',\', getCategory(' .
                     $catid . ',"arrparentid") . \',\' . ' . $catid . ')); ';
            $parsestr .= '  foreach ($arrparentid as $cid) {';
            $parsestr .= '      $parsestr[] = \'<li><a href="\' . getCategory($cid,\'url\')  . \'" ' .
                     $target .
                     '><i class="\'.getCategory($cid,\'caticon\').\'"></i> \' . getCategory($cid,\'catname\') . \'</a></li>\';';
            $parsestr .= '  }';
    ...
    public function _form ($tag, $content)
    {
        ...
        $parseStr = "<?php ";
        $parseStr .= ' echo call_user_func_array(array("\Form","' . $function .
                 '"),' . $parameter . ');';
     ...
     * 标签：<content></content>
     * 作用：内容模型相关标签，可调用栏目，列表等常用信息 * 用法示例：
     * <content action="lists" catid="$catid" order="id DESC" num="4"
     * page="$page"> .. HTML ..</content>
     * 参数说明： * 基本参数 * @action 调用方法（必填） * @page
     * 当前分页号，默认$page，当传入该参数表示启用分页，一个页面只允许有一个page，多个标签使用多个page会造成不可预知的问题。 * @num
     * 每次返回数据量 * @catid 栏目id（必填），列表页，内容页可以使用 $catid 获取当前栏目。 * 公用参数： * @cache
     * 数据缓存时间，单位秒 * @pagefun 分页函数，默认page * @pagetp 分页模板，必须是变量传递 * @return
     * 返回值变量名称，默认data * #当action为lists时，调用栏目信息列表标签 * #用法示例：
     * <content action="lists" catid="$catid" order="id DESC" num="4"
     * page="$page"> .. HTML ..</content>
     * 独有参数：
     * @order 排序，例如“id DESC”
     ...
     * #当action为hits时，调用排行榜
     * #用法示例：<content action="hits" catid="$catid" order="weekviews DESC"
     * num="10"> .. HTML ..</content>
     * 独有参数：
     ...
     * #用法示例：<content action="relation" relation="$relation" catid="$catid"
     * order="id DESC" num="5" keywords="$keywords"> .. HTML ..</content>
     * 独有参数：
     ...
     * #用法示例：<content action="category" catid="$catid" order="listorder ASC" >
     * .. HTML ..</content>
     * 独有参数：
     * @order 排序，例如“id DESC”
    ...
    public function _content ($tag, $content)
    {
        ...
        // 当前分页参数
        $tag['page'] = $page = (isset($tag['page'])) ? ((substr($tag['page'], 0, 
                1) == '$') ? $tag['page'] : (int) $tag['page']) : 0;
        // 分页函数，默认page
        $tag['pagefun'] = $pagefun = empty($tag['pagefun']) ? "page" : trim(
                $tag['pagefun']);
        // 数据返回变量
        ...
        $parseStr .= ' $content_tag = \Think\Think::instance("\Content\TagLib\Content");' .
                 "\r\n";
        // 如果有传入$page参数，则启用分页。
        if ($page && in_array($action, array(
                'lists'
        ))) {
            // 分页配置处理
            $pageConfig = $this->resolvePageParameter($tag);
            // 进行信息数量统计 需要 action catid where
            $parseStr .= ' $count = $content_tag->count(' .
                     self::arr_to_html($tag) . ');' . "\r\n";
            // 分页函数
            $parseStr .= ' $_page_ = ' . $pagefun . '($count ,' . $num . ',' .
                     $page . ',' . self::arr_to_html($pageConfig) . ');';
            $tag['count'] = '$count';
        ...
        $parseStr .= ' $' . $return . ' = $content_tag->' . $action . '(' .
                 self::arr_to_html($tag) . ');';
        $parseStr .= ' }';
     ...
     * 标签：<comment></comment>
     * 作用：评论标签
     * 用法示例：<comment action="get_comment" catid="$catid" id="$id"> .
     * . HTML ..</comment>
     * 参数说明：
     * 基本参数
     * @action 调用方法（必填）
     ...
     * @cache 数据缓存时间，单位秒
     * 
     * @return 返回值变量名称，默认data #当action为get_comment时，获取评论总数
     *         #用法示例：<comment action="get_comment" catid="$catid" id="$id"> ..
     *         HTML ..</comment>
     *         独有参数：
     *         @id 信息ID
     *         #当action为lists时，获取评论数据列表
     *         #用法示例：<comment action="lists" catid="$catid" id="$id"> .. HTML
     *         ..</comment>
     *         独有参数：
     *         @id 信息ID
     *         @hot 排序方式｛0：最新｝
    ...
    public function _comment ($tag, $content)
    {
        ...
        $parseStr .= ' $' . $return . ' = $comment_tag->' . $action . '(' .
                 self::arr_to_html($tag) . ');';
        $parseStr .= ' }';
        $parseStr .= ' ?>';
        $parseStr .= $this->tpl->parse($content);
     ...
     * 标签：<tags></tags>
     * 作用：Tags标签
     * 用法示例：<tags action="lists" tag="$tag" num="4" page="$page"
     ...
     * @cache 数据缓存时间，单位秒
     * 
     * @return 返回值变量名称，默认data @pagefun 分页函数，默认page()
     *         @pagetp 分页模板
     *         @where sql语句的where部分 例如：`child` = 0
     *         #当action为lists时，获取tag标签列表
     *         #用法示例：<tags action="lists" tag="$tag" num="4" page="$page"
     *         order="updatetime DESC"> .. HTML ..</tags>
     *         独有参数：
     ...
     *         #用法示例：<tags action="top" num="4" order="tagid DESC"> .. HTML
     *         ..</tags>
     *         独有参数：
     *         @num 每次返回数据量
     *         +----------------------------------------------------------
    ...
    public function _tags ($tag, $content)
    {
        ...
        $parseStr .= '     $' . $return . ' = $Tags_tag->' . $action . '(' .
                 self::arr_to_html($tag) . ');';
        $parseStr .= ' };';
     ...
     * 作用：推荐位标签
     * 用法示例：<position action="position" posid="1"> .
     * . HTML ..</position>
     * 参数说明：
     * 公用参数：
     * @cache 数据缓存时间，单位秒
     * 
     * @return 返回值变量名称，默认data @where sql语句的where部分
    ...
    public function _position ($tag, $content)
    {
        ...
        $parseStr .= '     $' . $return . ' = $Position_tag->' . $action . '(' .
                 self::arr_to_html($tag) . ');';
        $parseStr .= ' };';
        $parseStr .= ' ?>';
        $parseStr .= $this->tpl->parse($content);
     ...
     * 标签：<get></get>
     * 作用：特殊标签，SQL查询标签
     * 用法示例：<get sql="SELECT * FROM shuipfcms_article WHERE status=99 ORDER BY
     * inputtime DESC" page="$page" num="5"> .
     * . HTML ..</get>
     * 参数说明：
     * @sql
     * SQL语句，强烈建议只用于select类型语句，其他SQL有严重安全威胁，同时不建议直接在SQL语句中使用外部变量，如:$_GET,$_POST等。
     * @page 当前分页号，默认$page，当传入该参数表示启用分页，一个页面只允许有一个page，多个标签使用多个page会造成不可预知的问题。
     * @num 每次返回数据量
     * @cache 数据缓存时间，单位秒
     * 
    ...
    public function _get ($tag, $content)
    {
        ...
        // 每页显示总数
        $tag['num'] = $num = isset($tag['num']) && intval($tag['num']) > 0 ? intval(
                $tag['num']) : 20;
        // SQL语句
        if ($tag['sql']) {
            $tag['sql'] = $this->parseSqlCondition($tag['sql']);
        }
        $tag['sql'] = $sql = str_replace(array(
                "think_",
                "shuipfcms_"
        ), C("DB_PREFIX"), strtolower($tag['sql']));
        // 数据源
        ...
        if ($page) {
            if ($table) {
                ...
                $parseStr .= ' $cacheID = to_guid_string(array(' .
                         self::arr_to_html($tableWhere) . ',' . $page . '));';
                // 缓存处理
                ...
                // 取得信息总数
                $parseStr .= ' $count = $get_db->where(' .
                         self::arr_to_html($tableWhere) . ')->count();';
                $parseStr .= ' } ';
            } else {
                // 分析SQL语句
                if ($_sql = preg_replace('/select([^from].*)from/i', 
                        "SELECT COUNT(*) as count FROM ", $tag['sql'])) {
                    // 判断是否变量传递
                    if (substr(trim($sql), 0, 1) == '$') {
                        $parseStr .= ' $sql = str_replace(array("think_", "shuipfcms_"), C("DB_PREFIX"),' .
                                 $sql . ');';
                    ...
                    $parseStr .= ' $cache = ' . $cache . ';';
                    $parseStr .= ' $cacheID = to_guid_string(array($_sql,' .
                             $page . '));';
                    // 缓存处理
                    $parseStr .= ' if($cache && $_return = S($cacheID)){ ';
                    $parseStr .= ' $count = $_return["count"];';
            ...
            if ($table) {
                ...
                $parseStr .= '      $' . $return . ' = $get_db->where(' .
                         self::arr_to_html($tableWhere) .
                         ')->limit($_page_->firstRow.",".$_page_->listRows)->select();';
            } else {
                $parseStr .= '      $' . $return .
                         ' = $get_db->query($_sql." LIMIT ".$_page_->firstRow.",".$_page_->listRows." ");';
            }
            
            // 缓存处理
            $parseStr .= '      if($cache){ S($cacheID ,array("count"=>$count,"data"=>$' .
                     $return . '),$cache); }; ';
            $parseStr .= ' } ';
        } else {
            $parseStr .= ' $cache = ' . $cache . ';';
            if ($table) {
                $parseStr .= ' $cacheID = to_guid_string(' .
                         self::arr_to_html($tableWhere) . ');';
                $parseStr .= ' if(' . $cache . ' && $_return = S( $cacheID ) ){ ';
                ...
                $parseStr .= '      $' . $return . '=$get_db->where(' .
                         self::arr_to_html($tableWhere) . ')->limit(' . $num .
                         ')->select();';
            } else {
                $parseStr .= ' $cacheID = to_guid_string(' .
                         self::arr_to_html($tag) . ');';
                ...
                // 判断是否变量传递
                if (substr(trim($sql), 0, 1) == '$') {
                    $parseStr .= ' $_sql = str_replace(array("think_", "shuipfcms_"), C("DB_PREFIX"),' .
                             $sql . ');';
                } else {
                    $parseStr .= ' $_sql = "' . str_replace('"', '\"', $sql) .
                             '";';
                ...
                $parseStr .= ' $get_db = M();';
                $parseStr .= '      $' . $return .
                         '=$get_db->query($_sql." LIMIT ' . $num . ' ");';
            }
            $parseStr .= '      if(' . $cache . '){ S( $cacheID  ,$' . $return .
                     ',$cache); }; ';
            $parseStr .= ' } ';
        }
        $parseStr .= '  ?>';
     ...
     * 用法就是 <spf module="Demo" action="lists"> .
     * . HTML ..</spf> lists表示类DemoTagLib中一个public方法。
    ...
    public function _spf ($tag, $content)
    {
        ...
        // 拼接php代码
        $parseStr = '<?php';
        $parseStr .= '  import("' . $mo . 'TagLib", APP_PATH . "' . $mo .
                 '/TagLib/"); ';
        $parseStr .= '  $' . $mo . 'TagLib = \Think\Think::instance("\\' . $mo .
                 '\\TagLib\\' . $mo . 'TagLib"); ';
        // 如果有传入$page参数，则启用分页。
        if ($page) {
            // 分页配置处理
            $pageConfig = $this->resolvePageParameter($tag);
            // 进行信息数量统计 需要 action catid where
            $parseStr .= ' $count = $' . $mo . 'TagLib->count(' .
                     self::arr_to_html($tag) . ');' . "\r\n";
            // 分页函数
            $parseStr .= ' $_page_ = ' . $pagefun . '($count ,' . $num . ',' .
                     $page . ',' . self::arr_to_html($pageConfig) . ');';
        ...
        $parseStr .= ' $' . $return . ' = $' . $mo . 'TagLib->' . $action . '(' .
                 self::arr_to_html($tag) . ');';
        $parseStr .= ' }';
        $parseStr .= ' ?>';
        $parseStr .= $this->tpl->parse($content);
    ...
    {
        $condition = str_ireplace(array_keys($this->comparisonShuipfcms), 
                array_values($this->comparisonShuipfcms), $condition);
        return $condition;
    }
