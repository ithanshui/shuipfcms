  ...
  <div class="content-wrap"> 
    ...
    <div class="content">
    	...
    	<ul class="breadcrumb position">
            ...
            <li><a href="javascript:;"><i class="fa fa-tag"></i> {$SEO['title']|str_cut=###,2,''}</a></li>
        </ul>
        <!--栏目内容开始-->
        <content action="lists" catid="$catid" order="id DESC" num="7" page="$page">
        <div class="panel panel-default">
        	<div class="panel-heading">
        		<h2 class="panel-title">{$SEO['title']|str_cut=###,2,''}</h2>
        	</div>
            <ul class="panel-body">
            
           <volist name="list" id="vo" key="i" num="3">
           
                    <li class="list-blog">
						<h2 class="title"><a href="{$vo.url}">{$vo.info.title}</a></h2>
						<p class="flag">
						<span class="text-muted">
                        <i class="fa fa-tag" aria-hidden="true"></i> 
                        
                            <if condition="$i%5 eq 0">
                            <a href="{$vo.url}" class="label label-default">{$vo.tag}</a>
                            <elseif condition="$i%5 eq 1"/><span class="label label-primary">{$vo.tag}</span>
                            <elseif condition="$i%5 eq 2"/><span class="label label-success">{$vo.tag}</span>
                            <elseif condition="$i%5 eq 3"/><span class="label label-warning">{$vo.tag}</span>
                            <elseif condition="$i%5 eq 4"/><span class="label label-danger">{$vo.tag}</span>
                            <else/><span class="label label-info">{$vo.tag}</span>
                            </if>
                            
                        </span>
                        
						<span><i class="fa fa-eye"></i>：{$vo.usetimes}</span>
                        <span><i class="fa fa-clock-o"></i> {$vo.lasthittime|date='Y-m-d',###}</span>
                        
						<span class="pull-right text-pink hidden-xs hidden-sm">
						<i class="{:getCategory($vo['info']['catid'],'caticon')}"></i> 
                        <a href="{:getCategory($vo['info']['catid'],'url')}" class="btn btn-info btn-xs">					
                        	{:getCategory($vo['info']['catid'],'catname')}
                        </a>
						</span>
						</p>
						<article class="clearfix">
							<div class="thumbnail col-md-3 col-sm-4 col-xs-12"><img src="{:getOneField($vo['info']['catid'],$vo['info']['contentid'],'thumb')}" class="img-responsive img-block"></div>
							<div class="col-md-9 col-sm-8 col-xs-12">
								<p class="desc">{:getOneField($vo['info']['catid'],$vo['info']['contentid'],'description')}...</p>
