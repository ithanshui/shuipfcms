  ...
  <div class="content-wrap"> 
    ...
    <div class="content">
    	...
    	<ul class="breadcrumb position">
            ...
            <li><a href="{:U(Index/tag)}"><i class="fa fa-tag"></i> 标签</a></li>
        </ul>
        <!--栏目内容开始-->
        ...
        <div class="panel panel-default">
           ...
           <volist name="data" id="vo">           
                    <li class="list-blog">
						<h2 class="title"><a href="{$vo.url}" title="{$vo.title}">{$vo.title}</a></h2>
						<p class="flag">
                        ...
                        <span><i class="fa fa-clock-o"></i> {$vo.updatetime|date="m-d H:i:s",###}</span>
                        
						<span class="pull-right text-pink hidden-xs hidden-sm">
						<i class="{:getCategory($vo['catid'],'caticon')}"></i> 
                        <a href="{:getCategory($vo['catid'],'url')}" class="btn btn-info btn-xs">					
                        	{:getCategory($vo['catid'],'catname')}
                        </a>
						</span>
						</p>
						<article class="clearfix">
							<div class="thumbnail col-md-3 col-sm-4 col-xs-12"><img src="<if condition="$vo['thumb']">{$vo.thumb}<else />{$config_siteurl}statics/default/images/defaultpic.gif</if>" class="img-responsive img-block"></div>
