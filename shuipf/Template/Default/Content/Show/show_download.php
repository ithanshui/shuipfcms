      ...
      <article id="article">
        ...
        <div class="desc"><span class="time"><i class="fa fa-user text-blue"></i>
          <if condition="$username eq admin">三少爷
            <else/>
            网络</if>
          <i class="fa fa-eye text-blue"></i> <span id="hits text-blue">0</span> <i class="fa fa-clock-o text-blue"></i> {$updatetime|strtotime=###|date='m-d',###}</span></div>
          ...
          <if condition=" !empty($download) ">
            <ul class="tow-columns clearfix">
              <volist name="download" id="vo">
                <li class="l"><a href="{$vo.fileurl}" target="_blank" class="btn btn-success" title="下载所需积分{$vo.point}">{$vo.filename}</a></li>
              </volist>
            </ul>
          </if>
          <ul class="pagination">
        ...
        <div class="article-desc" style="margin-top:24px;"> {$description} </div>
      </article>
